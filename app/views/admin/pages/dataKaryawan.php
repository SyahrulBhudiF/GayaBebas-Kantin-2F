<section class="flex flex-col fadeIn p-4 gap-2 w-full h-screen">
    <div class="flex flex-col bg-Neutral/10 rounded-[1.25rem] p-6 gap-6 h-[87%] laptop3:h-[84vh] laptop2:h-[81vh]">
        <div class="flex justify-between">
            <label for="cari-karyawan" aria-label="search karyawan" title="search karyawan" class="flex justify-between items-center gap-1 px-7 py-3 border border-Neutral/30 rounded-full">
            ...
            <button id="addKaryawan" class="addButton" onclick="openModal('modal')" aria-label="tambah karyawan" title="tambah karyawan">Tambahkan</button>
        </div>
        <!-- Start table -->
                    ...
                    foreach ($data['karyawan'] as $karyawan) : ?>
                        <tr>
                            ...
                            <td class="tableContent flex justify-start gap-2">
                                <img onclick="openModal('modalEdit<?= $karyawan['id_user']; ?>')" src="../public/Assets/svg/edit.svg" alt="edit" class="iconKaryawan edit" aria-label="edit" title="edit">
                                <img onclick="openModal('modalDelete<?= $karyawan['id_user']; ?>')" src="../public/Assets/svg/trash.svg" alt="delete" class="iconKaryawan delete" aria-label="delete" title="delete">
                            </td>
                        </tr>
                    <?php $no++;
                    endforeach; ?>
                ...
                <div class="flex flex-col modal bg-white p-8 rounded-[2rem] shadow-lg gap-8 w-[28%] laptop1:w-[36%] laptop3:w-[42%]">
                    <div class="flex justify-between items-center">
                        ...
                        <button id="closeEdit" class="cursor-pointer">
                            <img onclick="closeModal('modalEdit<?= $karyawan['id_user']; ?>')" src="../public/Assets/svg/close.svg" alt="close">
                        </button>
                    </div>
            ...
            <div id="modalDelete<?= $karyawan['id_user']; ?>" class="fixed inset-0 items-center justify-center z-50 bg-black bg-opacity-60 hidden">
                <div class="flex flex-col modal bg-white p-8 rounded-[2rem] shadow-lg gap-8 w-[20%] laptop1:w-[27%] laptop3:w-[30%]">
                    ...
                    <div class="flex justify-between">
                        ...
                        <button class="px-[3.25rem] py-3 text-Neutral/100 bg-[#EEE] rounded-full" onclick="closeModal('modalDelete<?= $karyawan['id_user']; ?>')">Batal</button>
                    </div>
                </div>
            </div>
        <?php endforeach; ?>

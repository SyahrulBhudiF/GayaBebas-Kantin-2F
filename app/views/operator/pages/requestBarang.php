<section class="flex flex-col fadeIn p-4 gap-2 w-full h-screen">
    <div class="flex flex-col bg-Neutral/10 rounded-[1.25rem] p-6 gap-6 h-[87%] laptop2:h-[85%]">
        <div class="flex flex-col w-full gap-4">
            <div class="flex justify-between">
                <label for="cari-barang" class="flex justify-between px-7 py-3 border border-Neutral/30 rounded-full"
                    aria-label="Search by nama barang" title="search By nama barang">
                ...
                <div aria-label="Tombol untuk menambah barang" title="tambah barang baru">
                    <button class="py-3 px-5 rounded-full text-white bg-Primary-blue active:opacity-80"
                        onclick="openModal('addReqBarang')">Tambahkan</button>
                </div>
            </div>
        </div>
        <!-- Start table -->
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="tableContent flex justify-start gap-2">
                            <img onclick="openModal('editReqBarang<?= $request['id_request_barang']; ?>')"
                                src="../public/Assets/svg/edit.svg" alt="edit"
                                class="p-3 rounded-full border border-Neutral/40 edit <?= $request['status'] === 'Sedang Diproses' ? 'cursor-pointer' : 'notActiveReject'; ?>"
                                aria-label="edit request" title="edit">
                            <img onclick="openModal('deleteReq<?= $request['id_request_barang']; ?>')"
                                src="../public/Assets/svg/trash.svg" alt="delete"
                                class="p-3 rounded-full border border-Neutral/40 delete <?= $request['status'] === 'Sedang Diproses' ? 'cursor-pointer' : 'notActiveReject'; ?>"
                                aria-label="hapus request" title="hapus">
                        </td>
                    </tr>
                    <?php $no++;
                    endforeach; ?>
    ...
    <div id="addReqBarang" class="fixed inset-0 items-center justify-center z-50 bg-black bg-opacity-60 hidden">
        <div
            ...
            <form id="formAdd" class="flex flex-col gap-8 justify-center items-stretch"
                action="<?= BASEURL; ?>/operatorrequestbarang/tambahDataRequest" method="POST"
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex gap-2 w-full bg-Neutral/20 pr-6 rounded-xl">
                                <input type="text" name="date" id="date-input" class="inputKaryawan w-full"
                                    placeholder="Masukkan tanggal" onblur="blurDate('date-input')"
                                    onfocus="focusDate('date-input')">
    ...
    <div id="editReqBarang<?= $value['id_request_barang']; ?>"
        ...
        <div
            ...
            <div class="flex justify-between items-center">
                ...
                <button id="closeModal" class="cursor-pointer"
                    onclick="closeModal('editReqBarang<?= $value['id_request_barang']; ?>')">
                    <img src="../public/Assets/svg/close.svg" alt="close">
                </button>
            </div>
            <form id="formEdit" class="flex flex-col gap-8 justify-center items-stretch"
                action="<?= BASEURL; ?>/operatorrequestbarang/ubahDataRequestById/<?= $value['id_request_barang']; ?>"
                ...
                <div class="grid grid-cols-2 gap-8">
                    <div class="flex flex-col gap-3">
                        ...
                        <input type="text" name="nama" id="editNamaBarang" class="inputKaryawan"
                            placeholder="Masukkan nama barang" value="<?= $value['nama']; ?>">
                    </div>
                    <div class="flex flex-col gap-3">
                        <label for="editStok" class="textInputKaryawan">Jumlah Stok</label>
                        <input type="number" name="stok" id="editStok" class="inputKaryawan"
                            placeholder="Masukkan jumlah stok" value="<?= $value['stok']; ?>">
                    </div>
                    <div class="flex flex-col gap-3">
                        <label for="inputImg" class="textInputKaryawan">Upload gambar produk</label>
                        <div
                            class="flex flex-col items-center border-2 border-dashed border-Primary-surface rounded-3xl gap-2 cursor-pointer">
                            <label for="inputImgEdit<?= $value['id_request_barang']; ?>"
                                id="drop-area-edit<?= $value['id_request_barang']; ?>"
                                class="flex flex-col justify-center items-center m-2 px-[20%] py-[2%] h-[28vh] laptop1:h-[32vh] laptop3:h-[30vh] laptop2:h-[36vh]">
                                <div class="flex flex-col items-start w-fit">
                                    <div class="flex justify-end w-full -mb-2 cursor-pointer"
                                        onclick="cancelUpEdit<?= $value['id_request_barang']; ?>()">
                                        <img src="../public/Assets/svg/close.svg" alt="" class="-mr-[0.8rem]">
                                    </div>
                                    <img src="../../GayaBebas-Kantin-2F/public/Assets/img/barang/<?= $value['foto']; ?>"
                                        alt="" class="px-10 py-7 bg-Neutral/20 rounded-3xl">
                                </div>
                            </label>
                            <input type="file" name="inputImgEdit" id="inputImgEdit<?= $value['id_request_barang']; ?>"
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex gap-3">
                                <label for="editMakanan" class="inputRadio">
                                    <input type="radio" name="kategori" id="editMakanan" value="Makanan"
                                        <?= ($value['kategori'] == 'Makanan') ? 'checked' : ''; ?>>
                                    Makanan
                                </label>
                                <label for="editMinuman" class="inputRadio">
                                    <input type="radio" name="kategori" id="editMinuman" value="Minuman"
                                        <?= ($value['kategori'] == 'Minuman') ? 'checked' : ''; ?>>
                                    Minuman
                                </label>
                                <label for="editAtk" class="inputRadio">
                                    <input type="radio" name="kategori" id="editAtk" value="ATK"
                                        <?= ($value['kategori'] == 'ATK') ? 'checked' : ''; ?>>
                                    ATK
                                </label>
                            </div>
                        </div>
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="flex gap-2 w-full bg-Neutral/20 pr-6 rounded-xl">
                                <input type="text" name="date" id="date-input-edit" class="inputKaryawan w-full"
                                    placeholder="Masukkan tanggal" onblur="blurDate('date-input-edit')"
                                    onfocus="focusDate('date-input-edit')" value="<?= $value['tgl_expire']; ?>">
                                <img src="../public/Assets/svg/calendar-2.svg" alt="" class="date">
                            </div>
                        </div>
                        <div class="flex flex-col gap-3">
    ...
    <!-- end modal edit -->
    <!-- start modal delete -->
    <div id="deleteReq<?= $value['id_request_barang']; ?>"
        class="fixed inset-0 items-center justify-center z-50 bg-black bg-opacity-60 hidden">
        <div
            ...
            <div class="flex justify-between">
                <a
                    href="<?= BASEURL; ?>/operatorrequestbarang/hapusDataRequestById/<?= $value['id_request_barang']; ?>">
                ...
                <button class="px-[3.25rem] py-3 text-Neutral/100 bg-[#EEE] rounded-full"
                    onclick="closeModal('deleteReq<?= $value['id_request_barang']; ?>')">Batal</button>
            </div>
        </div>
    </div>
    <?php endforeach; ?>

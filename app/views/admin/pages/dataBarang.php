...
<section class="flex flex-col fadeIn p-4 gap-2 w-full h-screen">
    <div class="flex flex-col bg-Neutral/10 rounded-[1.25rem] p-6 gap-6 h-[87%] laptop2:h-[85%]">
        <div class="flex justify-between">
            <label for="cari-barang" aria-label="search barang" title="search barang"
            ...
            <div class="flex gap-3">
                <a href="<?= BASEURL; ?>/requestbarang">
                    <span id="request"
                        ...
                        aria-label="req barang" title="request barang">
                        <p>
                            Request Barang
                        </p>
                ...
                <button id="addBarang" class="addButton" onclick="openModal('addModalBarang')"
                    aria-label="tambah barang" title="tambah barang">Tambahkan</button>
            </div>
        </div>
        <!-- start grid -->
            ...
            <div class="flex flex-col border border-Neutral/30 rounded-3xl p-[0.2rem] div-barang">
                ...
                <div class="flex flex-col gap-3 p-4">
                    ...
                    <p class="text-Primary-blue text-xl font-semibold">Rp
                        <?= number_format($barang['hrg_jual'], 0, ',', '.'); ?><span
                ...
                <div class="flex gap-2 p-4 -mt-2 w-full">
                    <img src="../public/Assets/svg/trash.svg" alt="delete"
                        ...
                        onclick="openModal('deleteBarang<?= $barang['id_barang']; ?>')" aria-label="delete"
                        title="delete">
                    <button
                        class="EditItem w-full bg-Neutral/20 rounded-full text-center p-3 text-Primary-blue active:opacity-80"
                        onclick="openModal('editModalBarang<?= $barang['id_barang']; ?>')" aria-label="edit"
        ...
        <div id="addModalBarang" class="fixed inset-0 items-center justify-center z-50 bg-black bg-opacity-60 hidden">
            <div
                ...
                <form id="formAdd" class="flex flex-col gap-8 justify-center items-stretch"
                    action="<?= BASEURL; ?>/databarang/tambahDataBarang" method="POST" enctype="multipart/form-data">
        ...
        <!-- start modal edit -->
        <?php foreach ($data['barang'] as $barang) : ?>
        <div id="editModalBarang<?= $barang['id_barang']; ?>"
            ...
            <div
                ...
                <div class="flex justify-between items-center">
                    ...
                    <button id="closeModal" class="cursor-pointer"
                        onclick="closeModal('editModalBarang<?= $barang['id_barang']; ?>')">
                        <img src="../public/Assets/svg/close.svg" alt="close">
                    </button>
                </div>
                <form id="formEdit" class="flex flex-col gap-8 justify-center items-stretch"
                    action="<?= BASEURL; ?>/databarang/ubahDataBarangById/<?= $barang['id_barang']; ?>" method="POST"
                    ...
                    <div class="grid grid-cols-2 gap-8">
                        ...
                        <div class="flex flex-col gap-3">
                            ...
                            <div
                                ...
                                <label for="inputImgEdit<?= $barang['id_barang']; ?>"
                                    id="drop-area-edit<?= $barang['id_barang']; ?>"
                                    class="flex flex-col justify-center items-center m-2 px-[20%] py-[2%] h-[28vh] laptop1:h-[32vh] laptop3:h-[30vh] laptop2:h-[36vh]">
                                    <div class="flex flex-col items-start w-fit">
                                        <div class="flex justify-end w-full -mb-2 cursor-pointer"
                                            onclick="cancelUpEdit<?= $barang['id_barang']; ?>()">
                                            <img src="../public/Assets/svg/close.svg" alt="" class="-mr-[0.8rem]">
                                        </div>
                                        <img src="../../GayaBebas-Kantin-2F/public/Assets/img/barang/<?= $barang['foto']; ?>"
                                            alt="" class="px-10 py-7 bg-Neutral/20 rounded-3xl">
                                    </div>
                                </label>
                                <input type="file" name="inputImgEdit" id="inputImgEdit<?= $barang['id_barang']; ?>"
        ...
        <!-- start modal delete -->
        <?php foreach ($data['barang'] as $barang) : ?>
        <div id="deleteBarang<?= $barang['id_barang']; ?>"
            class="fixed inset-0 items-center justify-center z-50 bg-black bg-opacity-60 hidden">
            <div
                ...
                <div class="flex justify-between">
                    ...
                    <button class="px-[3.25rem] py-3 text-Neutral/100 bg-[#EEE] rounded-full"
                        onclick="closeModal('deleteBarang<?= $barang['id_barang']; ?>')">Batal</button>
                </div>
            </div>
        </div>
        <?php endforeach; ?>
